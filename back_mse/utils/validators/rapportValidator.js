...
exports.createrapportValidator=[
    ...
    check('Last_PM_Hours').notEmpty().withMessage('Last_PM_Hours required').isTime().withMessage('Last_PM_Hours required'),
    ...
    projetModel.findById(id_projet).then((projet) => {
          ...
          check('Technician_on_duty').isMongoId().withMessage('Invalid  Technician id ') 
          .custom((id_user) =>
          userModel.findOne({_id:id_user,role:"technicien"}).then((user) => {
                    if (!user) {
                      return Promise.reject(
                        new Error(`No technicien for this id: ${id_user}`)
                      );
                    }
                  })
                ),
